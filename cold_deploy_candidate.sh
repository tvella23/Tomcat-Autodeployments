...
##--------------------
#tomcatServiceName="tomcat7"  # Is an unique ID of tomcat service.
...
# the process id of the tomcat service running on the target server 
...
"${ERP_BIN_DIR}"/download_candidate.sh \
                       "${DOWNLOAD_DIR}" \
                       "${LOG}" \
                       "${REPO_URL}" \
                       "${REPO_ID}" \
...
if [ $? -eq $SUCCESS ] ; then
   ...
   # tomcat manager deployer user id
   DEPLOYER_USER_ID="NOT_APPLICABLE"

   # tomcat manager deployer password
   DEPLOYER_PASSWORD="NOT_APPLICABLE"
